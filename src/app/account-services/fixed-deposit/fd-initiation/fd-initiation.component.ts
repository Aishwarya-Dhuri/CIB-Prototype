...
import { FdInitiation } from './@models/fd-initiation.model';

@Component({
  selector: 'app-fd-initiation',
  templateUrl: './fd-initiation.component.html',
  styleUrls: ['./fd-initiation.component.scss'],
})
export class FdInitiationComponent implements OnInit {
  @ViewChild('fdInitiationForm') fdInitiationForm: any;
  @Input('isViewMode') isViewMode: boolean = false;

  loading: boolean = false;
  formData: FdInitiation = new FdInitiation();
  mode: string;
  currencyList: any[] = [];
  isFDInitiated: boolean = false;
  ...
  getDropdownData(): void {
    ...
    this.httpService
      .httpPost('accountServices/fixedDeposit/fdInitiation/private/dropdown/yearsDataList', data)
      .subscribe((response: any) => {
        this.yearsDataList = response.dataList;
      });

    this.httpService
      .httpPost('accountServices/fixedDeposit/fdInitiation/private/dropdown/monthsDataList', data)
      .subscribe((response: any) => {
        this.monthsDataList = response.dataList;
      });

    this.httpService
      .httpPost('accountServices/fixedDeposit/fdInitiation/private/dropdown/daysDataList', data)
      .subscribe((response: any) => {
  ...
  getViewData(): void {
    ...
    if (this.mode == 'EDIT' || this.mode == 'VIEW') {
      ...
      this.httpService
        .httpPost('accountServices/fixedDeposit/fdInitiation/private/view', data)
        .subscribe((formData: FdInitiation) => {
  ...
  validateForm(stepNo: number): boolean {
    if (stepNo == 1) {
      if (this.fdInitiationForm && this.fdInitiationForm.valid) {
        return true;
      }
      return false;
    }
