...
export class AccountStatementComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.viewService.getMode() == 'PREFILLED') {
      ...
      this.searchFilters[1].value = data.id;
  ...
  onChangedateRangeOption(fp: string, accountCalender: any) {
    ...
    this.accountStatmentService.selectedAccountPeriodOption = fp;
  ...
  onChangeAccountPeriod() {
    if (this.accountPeriod[0] && this.accountPeriod[1]) {
      ...
      this.accountStatmentService.accountPeriod = accountPeriod.join(' - ');
    }
  }
  ...
  applyCriteria(criteria: Criteria): void {
    ...
    this.onAccountSelected(this.searchFilters[1].value);
    if (
  ...
  resetFilters(): void {
    this.searchFilters = this.accountStatmentService.searchFilters;

    if (!this.searchFilters || (this.searchFilters && this.searchFilters.length == 0)) {
      this.searchFilters = this.accountStatmentService.getResetFilters();
    }
  ...
  viewOnScreenClick(): void {
    this.searchFilters[1].value
      ? (this.accountStatmentService.selectedAccount = this.searchFilters[1].value)
      : (this.accountStatmentService.selectedAccount = this.searchFilters[2].value);

    this.accountStatmentService.searchFilters = this.searchFilters;

    this.router.navigateByUrl('/accountServices/services/accountStatementView');
  }

  onAccountSelected(account: any): void {
    if (this.searchFilters[0].value == 'IBAN') {
      this.accountStatmentService.selectedAccountNumber = this.ibanAccountList.find(
        (res: any) => res.id === account.id,
      )?.enrichments.accountNo;
      this.searchFilters[2].value = account.id;
      this.searchFilters[2].displayValue = account.displayName;
    } else {
      this.accountStatmentService.selectedAccountNumber = this.accountsList.find(
        (res: any) => res.id === account.id,
      )?.enrichments.accountNo;
      this.searchFilters[1].value = account.id;
      this.searchFilters[1].displayValue = account.displayName;
    }
  }
}
