...
import { GroupSelect, Select } from '../../@models/select.model';

@Component({
  selector: 'aps-group-select',
  templateUrl: './aps-group-select.component.html',
  styleUrls: ['./aps-group-select.component.scss'],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => ApsGroupSelectComponent),
      multi: true,
    },
  ],
})
export class ApsGroupSelectComponent implements OnInit, OnChanges, ControlValueAccessor {
  ...
  set value(val) {
    ...
    if (this.data) {
      for (let gd of this.data) {
        const gdt: Select = gd.items.find((dt: Select) => dt.id === this.val);
