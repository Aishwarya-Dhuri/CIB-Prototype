...
@Component({
  selector: 'app-danger-currency-renderer',
  template: `
    <div class="text-color-danger" [ngClass]="{ 'text-right': isAgGridRenderer }" *ngIf="cellValue">
      <app-base-currency [amount]="cellValue" [currency]="currencyName"></app-base-currency>
    </div>
    <div *ngIf="!cellValue">-</div>
  `,
})
export class DangerCurrencyRendererComponent implements OnInit, AgRendererComponent {
  cellValue!: any;
