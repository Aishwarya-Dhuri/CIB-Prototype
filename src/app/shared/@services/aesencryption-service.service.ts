...
export class AESEncriptionService {
  aesKey: string;
  hexcase = 0;
  chrsz = 8;

  constructor() {
    this.aesKey = environment.aesKey;
  }

  public doEncryption(text: string): string {
    return this.hex_md5(text);
  }
  ...
  binl2hex(binarray: number[]): string {
  ...
  core_md5(x: number[], len: number): number[] {
    /* append padding */
    x[len >> 5] |= 0x80 << len % 32;
    x[(((len + 64) >>> 9) << 4) + 14] = len;
