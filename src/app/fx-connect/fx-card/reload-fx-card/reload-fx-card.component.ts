...
import { ReloadFxCard, CurrencyDetails } from './@model/reloadFxCard.model';

@Component({
  selector: 'app-reload-fx-card',

  templateUrl: './reload-fx-card.component.html',
  styleUrls: ['./reload-fx-card.component.scss'],
})
export class ReloadFxCardComponent implements OnInit {
  @ViewChild('fxForm') fxForm: any;
  ...
  formData: ReloadFxCard = new ReloadFxCard();
  ...
  colDefUrl: string = 'fxConnect/fxCard/reload/private/colDefs';
  ...
  ngOnInit(): void {
    ...
    const actions: Actions = {
      heading: 'FX Card - Reload - Initiate',
      subHeading: null,
  ...
  getViewData() {
    ...
    if (this.mode == 'EDIT' || this.mode == 'VIEW') {
      ...
      this.httpService
        .httpPost('fxConnect/fxCard/reload/private/view', data)
  ...
  getCardDetails() {
    if (this.formData.fxCardNo) {
      const data = { dataMap: { id: this.formData.fxCardNo } };
      this.httpService
        .httpPost('fxConnect/fxCard/buy/private/view', data)
        .subscribe((formData: any) => {
          if (formData) {
            this.formData = {
              fxCardNo: this.formData.fxCardNo,
              cardBalance: this.formData.cardBalance,
  ...
  validateForm(stepNo: number) {
    if (stepNo == 1) {
      return (
        this.formData.fxCardNo &&
        this.formData.cardBalance &&
        this.formData.expiryMonth &&
        this.formData.expiryYear &&
        this.formData.countryOfTravel &&
        this.formData.purpose &&
        this.formData.activationDate &&
        this.formData.embossedName &&
        this.formData.mobileNo &&
        this.formData.emailId &&
        this.formData.deliveryDetails &&
        ((this.formData.deliveryDetails == 'Courier me' &&
          this.formData.addressType == 'Other' &&
          this.formData.address1) ||
          (this.formData.deliveryDetails == 'Courier me' &&
            this.formData.addressType == 'Corporates Registered Address') ||
          (this.formData.deliveryDetails == 'I will collect' &&
            this.formData.branch &&
            this.formData.collectingOfficialsName &&
            this.formData.collectingOfficialsIdType &&
            this.formData.idCardNumberDetails))
      );
    } else if (stepNo == 2) {
      return this.isAgree;
    }
  ...
  getSupportingDocuments() {
    this.httpService
      .httpPost('fxConnect/reload/private/getSupportingDocumentsFields')
  ...
  getTermsAndConditions() {
    this.httpService
      .httpPost('fxConnect/reload/private/getTermsAndConditions')
      .subscribe((response: any) => {
