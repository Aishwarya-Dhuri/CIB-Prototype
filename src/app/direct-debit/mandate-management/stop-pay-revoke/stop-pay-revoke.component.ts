...
export class StopPayRevokeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.mode == 'EDIT' || this.mode == 'VIEW') {
      ...
      this.httpService
        .httpPost('directDebit/mandateManagement/stopPayRevoke/private/view', data)
  ...
  holddaysCalc() {
    var startDateCalc = new Date(this.formData.startDate);
    var endDateCalc = new Date(this.formData.endDate);

    var Difference_In_Time = endDateCalc.getTime() - startDateCalc.getTime();

    // To calculate the no. of days between two dates 
    this.formData.holdDays = Difference_In_Time / (1000 * 3600 * 24);
